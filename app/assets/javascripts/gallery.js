(function(a){
  a.fn.authorcode_scroller_amazon=function(p){
    var p=p||{};
    ...
    var q=p&&p.scroller_title_color?p.scrooler_title_color:"blue";
    ...
    var x=Math.ceil(w/r);
    if(dom.find("ul").length==0||find("li").length=0){
    ...
    s_s_ul(dom,j,k,l,i);
    s_s_nav(dom.find(".authorcode_scroller_simple2_nav"),d);
    m=parseInt(m);
    ...
    begin();
    play();
    dom.find(".authorcode_scroller_simple2_nav").children("li").click(function(){
  ...
  function begin(){
    ...
    dom.width(u+60);
    dom.height(v);
    dom.children(".authorcode_scroller_simple2_mask").width(u);
    dom.children(".authorcode_scroller_simple2_mask").height(v);
    dom.find("ul:first").children("li").show();
    dom.css("position","relative");
    dom.find("ul:first").css("position","absolute");
    dom.children(".authorcode_scroller_simple2_mask").width(u);
    dom.children(".authorcode_scroller_simple2_mask").height(v);
    dom.find(".authorcode_scroller_simple2_nav").css('top',(v-50)/2+parseInt(j)+"px");
    dom.find(".authorcode_scroller_simple2_nav").width(u+60)
  }
  function previous(){
    if (t==0){
      t=x-2
